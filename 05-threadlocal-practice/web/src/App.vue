...
<template>
  <div class="page">
    <header class="site-header">
      <div class="title-row">
        <h1>05 · ThreadLocal 全场景实践</h1>
        <span class="badge">{{ totalScenarios }} 个场景 / {{ modules.length }} 个模块</span>
      </div>
      <p class="subtitle">
        逐个运行 ThreadLocal 实验场景，理解线程隔离、Web 上下文、MDC 链路日志、线程池串号、TTL 透传与内存泄漏原理。
      </p>
      <p class="backend-hint">后端地址：<code>http://localhost:8085</code>（开发环境由 Vite 代理 /api 转发）</p>
    </header>
