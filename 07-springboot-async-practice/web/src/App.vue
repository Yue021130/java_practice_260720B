...
<template>
  <div class="page">
    <header class="site-header">
      <div class="title-row">
        <h1>07 · Spring Boot 异步任务与线程池</h1>
        <span class="badge">{{ totalScenarios }} 个场景 / {{ modules.length }} 个模块</span>
      </div>
      <p class="subtitle">
        逐个运行 @Async、ThreadPoolTaskExecutor、拒绝策略、异常处理、上下文透传、批量聚合等实验，理解 Spring Boot 异步编程与面试八股。
      </p>
      <p class="backend-hint">后端地址：<code>http://localhost:8087</code>（开发环境由 Vite 代理 /api 转发）</p>
    </header>
