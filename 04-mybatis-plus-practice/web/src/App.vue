...
<template>
  <div class="page">
    <header class="site-header">
      <div class="title-row">
        <h1>04 · Spring Boot + MyBatis-Plus 全场景实践</h1>
        <span class="badge">{{ totalScenarios }} 个场景 / {{ modules.length }} 个模块</span>
      </div>
      <p class="subtitle">
        逐个运行 MyBatis-Plus 实验场景，观察实体注解、BaseMapper、IService、条件构造器、分页、逻辑删除、乐观锁等行为与面试考点。
      </p>
      <p class="backend-hint">后端地址：<code>http://localhost:8084</code>（开发环境由 Vite 代理 /api 转发）</p>
    </header>
