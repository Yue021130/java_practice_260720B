 ...
 * 大部分「失败」其实是教学演示（如等待超时 / 参数非法），都在 Service 内部捕获
...
public class GlobalExceptionHandler {
    ...
    /**
     * 线程通信业务异常：已知错误，返回错误码与提示。
     */
    @ExceptionHandler(CommBizException.class)
    public ResponseEntity<ApiResponse<Void>> handleCommBizException(CommBizException e) {
        log.warn("线程通信业务异常：{}", e.getMessage());
